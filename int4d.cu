...
void f1(double**** fxyzt, double* lZ, int n, double m)
{
	...
	for (i = 0; i < n; i++)
	{
		double t0 = sin(M_PI*lZ[i]/2.0)*sin(M_PI*lZ[i]/2.0);
		for (j = 0; j < n; j++)
		{
			double t1 = sin(M_PI*lZ[j]/2.0)*sin(M_PI*lZ[j]/2.0);
			for (k = 0; k < n; k++)
			{
				double t2 = sin(M_PI*lZ[k]/2.0)*sin(M_PI*lZ[k]/2.0);
				for (l = 0; l < n; l++)
				{
					double t3 = sin(M_PI*lZ[l]/2.0)*sin(M_PI*lZ[l]/2.0);
					fxyzt[i][j][k][l] = ((t0 + t1 + t2 + t3 + m2)*(t0 + t1 + t2 + t3 + m2));
				}
			}
		}
	}
