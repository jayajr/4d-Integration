...
void init_all(double* lZ, double** gQ_SoE, double* gQ_Soln, double* sinVals, int n, double m);
...
`	*/
	...
	init_all(lZ, gQ_SoE, gQ_Soln, sinVals, n, m);
#if TIME_CODE
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
	elapsed = diff(time1, time2);
...
*********************************/
	...
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
#endif
	cleanUp(lZ, gQ_SoE, gQ_Soln, sinVals, n);
#if TIME_CODE
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
	elapsed = diff(time1, time2);
